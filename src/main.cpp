...
{
    // this is provided by boost::program_options
    options_description opt("Options");

    //option name , option value  and option description
    opt.add_options()
        ("delay,d",     value<std::string>(), "set a delay in ms")
        ("no-output,o", "never display the output to show the progress")
        ("num,n",       value<std::string>(), "set number of packets to send")
    ...
    //Inbuilt data structure provided by boost::program
    variables_map vmap;

    //again provided by boost::program options
...
int main(int argc, char **argv)
{
    int i = 0;
    try {
        //Setting the seed option while using rand function
        ...
        for( ; i < lexical_cast<int>(argmap["num"]); ++i ) {
            ...
            //.data returns all charachter arrays in ConstBufferSequence
            socket.send_to(request_buffer.data(), destination);
            if( argmap["no-output"].empty() ) 
                std::cout << '.' << std::flush;

            // Remove charachters from input sequence
